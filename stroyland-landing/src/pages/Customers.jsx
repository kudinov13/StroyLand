...
const DEFAULT_CONTENT = {
  title: 'Заказчикам',
  lead:
    'Уважаемые руководители и менеджмент строительных организаций! ООО «Литокс» предлагает аутсорсинг функций отдела снабжения. Просим оставить заявку на нашем сайте и мы свяжемся с вами для уточнения условий возможного сотрудничества.',
  benefitsTitle: 'Наши преимущества:',
  benefitsItems: [
    'Снижение затрат на закупки до 15%',
    'Оперативная поставка материалов',
    'Полный контроль и прозрачность',
    'Опыт более 20 лет на рынке',
  ],
  extraBlocks: [],
  primaryAction: 'Условия для заказчиков',
...
function Customers() {
  ...
  useEffect(() => {
    const unregister = registerPage('customers', { save, reset, isDirty: () => isDirty })
        ...
        <div className="editable-block">
          ...
          {editing.main && isAdmin ? (
            <div className="edit-fields">
              ...
              <label>
                Текст
                <textarea rows={4} value={content.lead} onChange={updateField('lead')} />
              </label>
              <label>
                Заголовок списка преимуществ
                <input value={content.benefitsTitle} onChange={updateField('benefitsTitle')} />
              </label>
              <div className="edit-list">
                <span>Список преимуществ</span>
                {content.benefitsItems.map((item, index) => (
                  <div key={`${item}-${index}`} className="edit-list-row">
                    <input value={item} onChange={updateListItem('benefitsItems', index)} />
                    <button className="ghost-btn" type="button" onClick={removeListItem('benefitsItems', index)}>
                      Удалить
                    </button>
                  </div>
                ))}
                <button className="secondary-btn" type="button" onClick={addListItem('benefitsItems')}>
                  Добавить строку
                </button>
              </div>
              <div className="edit-list-actions">
                <button className="ghost-btn" type="button" onClick={() => updateField('benefitsTitle')({ target: { value: '' } })}>
                  Удалить заголовок
                </button>
            ...
            <>
              ...
              <h3 className="page-subtitle">{content.benefitsTitle}</h3>
